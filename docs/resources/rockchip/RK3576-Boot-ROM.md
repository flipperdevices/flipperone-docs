...
- Code `0x472`. This is meant for the code which relies on normal system RAM already up and running, such as the U-Boot SPL binary.
...
<table isTableHeaderOn="true" columnWidths="85,85,365,155">
  ...
  <tr>
    ...
    <td>
      <p><code>IRAM+0xb4</code> bit 4 → run <code>vops[1]</code> = <code>0x620c</code> after link startup: PA_TxGear=3, PA_HSSeries=2 (Rate B), <code>PA_PWRMode=0x44</code> → HS-G3 Rate-B, 2 lanes</p>
    </td>
    <td>
